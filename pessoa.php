            ...
            if(!isset($_POST['procura']) && !isset($_POST['letra'])){
                ...
                switch($cases){
                    case($cases[0] !=="" && $cases[1] == ""):
                        $sql = mysqli_query($conn, "SELECT * FROM pessoa WHERE nome LIKE '".$busca."%' ORDER BY nome") or die (mysqli_error($conn));
                    break;
                    case($cases[1] !== ""):
                        $sql = mysqli_query($conn, "SELECT * FROM pessoa WHERE nome LIKE '".$busca2."%' ORDER BY nome") or die (mysqli_error($conn));
                    break;
                    case($cases[0] =="" && $cases[1] == ""):
                        $sql = mysqli_query($conn, "SELECT * FROM pessoa ORDER BY nome") or die (mysqli_error($conn));
                    break;
                }
